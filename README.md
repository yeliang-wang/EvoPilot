...
EvoPilot 是受控多租户控制台，不提供公网自助注册入口。账号开通链路是 `平台高级管理员 -> 租户管理员 -> 租户内用户`：平台高级管理员首次使用 `admin/admin` 登录并改密后创建租户、工作区和租户管理员；租户管理员再在“用户与权限”中创建本租户开发者、发布负责人和 Viewer。未登录用户只能看到登录页、公开帮助和健康检查，不能创建租户、接入项目或读取租户数据。
...
| 用户与权限 | 按云平台 IAM 方式管理账号、角色和作用域。平台高级管理员可创建租户用户、停用/启用账号、重置密码和查看全平台权限矩阵；租户管理员只能管理本租户用户。 |
| 工作区 | 展示 workspace 成员、角色、项目、Loop、发布证据和 SaaS targets，并保留 Source-to-GA 向导、Target Runtime、Target Loop Backlog。 |
| 项目 | 通过 GitHub、GitLab 或本地 Git 注册项目，验证源码归属和项目级工作区；项目详情页提供 Experimental、Alpha、Beta、RC、GA 发布等级模板，用户可复制为项目专属 target 并生成该项目自己的 release decision。 |
...
| Loops | 默认把 Loop 分为“当前、待处理、历史”三组，优先显示下一步和状态；需要 trace、replay、sandbox、worker queue、Workflow Canvas 或 AI 日志诊断时再进入高级控制台。 |
...
| 帮助手册 | 按公有云帮助中心的信息架构展示文档中心、本页目录、快速上手流程、角色权限、操作场景、前提条件、操作步骤、结果验证、后续操作和相关 API；已覆盖 AI 辅助日志诊断与故障定位流程。 |

旧入口仍保持兼容：`首页` / `工作台` 映射到 `租户总览`，`用户` / `权限` / `IAM` / `访问控制` 映射到 `用户与权限`，`主链路` / `主链路向导` / `向导` 映射到 `工作区`，`项目接入` / `接入项目` 映射到 `项目`，`证据策略` / `发现与目标` / `评测集` / `机会点` 映射到 `工作区`，`Loop` / `Loop 执行` 映射到 `Loops`，`流水线` / `发布` / `评估与发布` 映射到 `发布证据`，`历史记录` / `历史审计` / `历史` 映射到 `审计`。

Loops 页不再把所有运行细节堆在一张长页。默认页回答“现在该处理哪个 Loop”，并在没有运行记录时直接给出可点击模板，而不是只展示空状态。高级控制台仍提供总览、详情和创建三类工作区：总览页处理 Target Backlog、Loop Runtime 列表和 Worker Queue；Loop 详情页回答“这个 Loop 走到哪一步、为什么卡住、下一步做什么”；创建页回答“如何创建或调整新的 Loop graph”。真实验收时应从概览行点击“打开 Loop 详情”，在单个 loopId 的详情页核对项目、Human Gate、LLM provider/model/tokens、trace 和 release evidence。`WAITING_APPROVAL` 是正常 Human Gate，表示等待人工批准，不按超时失败处理。详情页里的 Source-to-GA 动态本体链路图不是静态流程图；它把当前选中的 LoopRun 与项目、Discovery、Target Backlog、ExecutorGraph、worker/sandbox、human gate、`sourceClosure`、deploy finalizer、`sourceReleaseRun` 和 release decision 合成一条可读链路：`SCM/Git Project -> Discovery Candidate -> Target Backlog -> Executor Graph -> Worker + Sandbox -> Human Gate -> Source Closure -> CI/CD + Deploy -> Release Decision -> GA Release`。

生产部署首次读取 `GET /api/v1/evaluation-datasets` 时会持久化三条生产基线评测集：`prod-baseline-source-to-ga`、`prod-baseline-tenant-rbac` 和 `prod-baseline-worker-human-gate`。它们不是 sample/mock 数据，而是干净生产环境的基础验收入口；真实项目接入和运行信号会继续追加项目自己的评测集。

租户总览会展示 workspace 数量、项目数、凭据阻塞、发布证据、Loop 配额、项目配额和 evidence storage 配额，同时提供 `GitHub 项目到 GA Release`、`已有 CI 项目接入`、`失败发布修复`、`EvoPilot 自演进` 四类模板入口。平台高级管理员还可以在租户总览完成租户与初始工作区开通。用户与权限页读取持久化用户目录，支持 `admin/admin` bootstrap 后强制改密、创建租户管理员和租户内用户、角色分配、启停账号和重置密码。

项目级发布治理采用“标准等级模板 + 项目专属 target”的方式。EvoPilot 内置 `experimental`、`alpha`、`beta`、`rc`、`ga` 五个等级模板；用户接入 GitHub 项目后，可在项目页把模板复制为 `scope=project` 的 release target，再按该项目的 pipeline、code upgrade、source release run、risk 和 scenario evidence 生成独立 `GO` / `CONDITIONAL-GO` / `NO-GO` 判定。项目级 target 带 `projectId`，不能被其他项目借用；`GET /api/v1/release/decisions?targetId=<target>&projectId=<project>` 可只查询某项目的判定历史。
...
服务端生产日志使用 `evopilot-log/v1` JSON Lines，包含 `tenantId`、`workspaceId`、`correlation`、`routeGroup`、`latencyBucket`、`outcome` 和 `diagnosis.recommendedAction`，便于 GLM、Codex 或日志平台按请求、Loop、Release Run 聚合排障。部署和查询示例见 [docs/deployment.md](docs/deployment.md)。
...
- 生产环境可以使用持久化 bootstrap 管理员、`EVOPILOT_USERS`、`EVOPILOT_TOKENS` 或 `EVOPILOT_API_TOKEN` 建立鉴权边界。Dashboard 面向最终用户使用用户名/密码登录；首次持久化 bootstrap 账号为 `admin/admin`，登录后必须改密。普通用户账号由平台高级管理员或租户管理员创建，不提供公网自助注册。
...
| `EVOPILOT_USERS` | 可选的 Dashboard 登录用户种子，格式为 `username:password:role:tenantId:workspaceId[:displayName][:platformAdmin]`；运行中创建的用户会写入持久化用户目录。 |
