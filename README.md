...
租户总览会展示 workspace 数量、项目数、凭据阻塞、发布证据、Loop 配额、项目配额和 evidence storage 配额，同时提供 `GitHub 项目到 GA Release`、`已有 CI 项目接入`、`失败发布修复`、`EvoPilot 自演进` 四类模板入口。平台高级管理员还可以在租户总览完成租户与初始工作区开通。用户与权限页读取持久化用户目录，支持 `admin/admin` bootstrap 后强制改密、创建租户用户、角色分配、启停账号和重置密码。
...
- 生产环境可以使用持久化 bootstrap 管理员、`EVOPILOT_USERS`、`EVOPILOT_TOKENS` 或 `EVOPILOT_API_TOKEN` 建立鉴权边界。Dashboard 面向最终用户使用用户名/密码登录；首次持久化 bootstrap 账号为 `admin/admin`，登录后必须改密。
