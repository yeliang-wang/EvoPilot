...
CLI 也可以操作同一套服务端对象，适合 WorkBuddy、Codex、Claude Code 等外部 AI Agent 接入。傻瓜式 wrapper command 和生产接入手册见 [CLI Manual](cli-manual.md)；需要逐步控制时可以使用原子命令：
...
8. `CI/CD + Deploy`：确认项目 DevOps、pipeline evidence、deploy connector、post-merge deploy、health/ready 和 rollback/finalizer 证据是否完整。Jenkins 只用于旧部署兼容，不是新 GitHub/GitLab 项目的必需链路。
