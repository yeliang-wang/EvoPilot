...
没有配置 `deployConnectorId` 时，部署 gate 仍兼容旧行为：只记录 `deploymentUrl` 并探测 health/ready URL。配置部署连接器后，deploy gate 必须由连接器真实返回成功才会 `PASSED`。ECS/K8s/云发布执行器应接入该 deploy connector contract，而不是在 source closure 里硬编码平台逻辑。
